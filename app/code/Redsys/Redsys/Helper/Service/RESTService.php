...
namespace Redsys\Redsys\Helper\Service;

use Redsys\Redsys\Helper\Model\RESTResponseInterface;
...
abstract class RESTService {
	...
	public function sendOperation($message, $idLog = null) {
		$result="";
		...
		RedsysLibrary::escribirLog("INFO ", $idLog, "Realizando petición a: " . $url_rs, null, __METHOD__);
		...
		if($tmp !== false && $httpCode==200){
			...
			RedsysLibrary::escribirLog("ERROR", $idLog, "Se ha producido un error en la petición - " . $strError, null, __METHOD__);
		}
	...
	public function createRequestSOAPMessage($message, $idLog = NULL) {
		...
		RedsysLibrary::escribirLog("DEBUG", $idLog, "Datos de la petición (en XML): " . $request->toXml(), null, __METHOD__);
	...
	protected function checkSignature($sentData, $remoteSignature, $idLog = NULL) {				
		...
		if(!$result) {
			RedsysLibrary::escribirLog("ERROR", $idLog, "La firma remota y la firma calculada no coincide", null, __METHOD__);
			RedsysLibrary::escribirLog("DEBUG", $idLog, "Firma recibida : " . $remoteSignature, null, __METHOD__);
			RedsysLibrary::escribirLog("DEBUG", $idLog, "Firma calculada: " . $calcSignature, null, __METHOD__);
		} else {
			RedsysLibrary::escribirLog("DEBUG", $idLog, "Firmas correctas", null, __METHOD__);
		}
