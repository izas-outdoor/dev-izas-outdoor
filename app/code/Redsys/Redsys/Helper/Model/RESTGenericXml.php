...
use Redsys\Redsys\Helper\Utils\RESTAnnotation;
		...
		if($thisTag !== NULL){
			...
			foreach($thisClass->getProperties() as $prop){
				...
				if($xmlClass !== NULL){
					...
					if($obj !== NULL){
						$propClass=new \ReflectionClass(get_class($obj));
			...
			if ($thisClass->hasProperty("parameters")) {
				$valores=$this->getPropertyValue($thisClass->getProperty("parameters"));
					if($valores!=null){
