...
class Product extends \Magento\CatalogImportExport\Model\Export\Product
{
    ...
    protected function getExportData()
    {
        ...
        if($this->_storeId == 8){
            ...
            $endpoint = 'latest';
            $access_key = '3fb2c313ee16f99842fd136d93ccc3cc';
            ...
            foreach ($productData as $dataRow) {
                if($this->_storeId == 8){
                    if($exchangeRates['success']) {
                        ...
                        try {
                            // Decoding
                            if(isset($dataRow['dfw_price'])){
                                $dataRow['dfw_price'] = round(($dataRow['dfw_price'] * $exchangeRates['rates']['CLP']), 0, PHP_ROUND_HALF_UP);

                            }
                            if(isset($dataRow['special_price'])){
                                $dataRow['special_price'] = round(($dataRow['special_price'] * $exchangeRates['rates']['CLP']), 0, PHP_ROUND_HALF_UP);
                ...
                if(isset($dataRow['special_to_date']) && isset($dataRow['special_from_date'])){
                    if($fecha_actual > strtotime($dataRow['special_to_date']) || $fecha_actual < strtotime($dataRow['special_from_date'])){
                        $dataRow['special_price'] = '';
                    }
                }
