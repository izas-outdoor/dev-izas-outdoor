...
namespace Seonov\Clubredirect\Controller\Index;
...
class Index extends \Magento\Checkout\Controller\Onepage implements HttpGetActionInterface
{
    ...
    public function execute()
    {
        ...
        if (!$this->_customerSession->isLoggedIn() && !$checkoutHelper->isAllowedGuestCheckout($quote)) {
            $this->messageManager->addErrorMessage(__('TO PURCHASE KILPI CLUB - PLEASE LOGIN.'));
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $urlInterface = $objectManager->create('Magento\Framework\UrlInterface');

            $url = $urlInterface->getUrl('checkout/index');
