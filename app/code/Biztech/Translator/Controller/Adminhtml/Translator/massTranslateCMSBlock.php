...
class massTranslateCMSBlock extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if (array_key_exists('selected', $params)) {
            ...
            $cmsBlock = $this->_blockFactory->create()->getCollection()->addFieldToFilter('block_id', ['in' => $implodeIds ]);
        } else {
            //$cmsBlock = $this->_blockFactory->create()->getCollection();
            $cmsBlock = $this->filter->getCollection($this->collectionFactory->create());
        }
        ...
        try {
            ...
            foreach ($cmsBlock as $item) {
                foreach ($finalFields as $attributeCode) {
                    ...
                    $translate = $this->_translatorModel->getTranslate($contentdata, $langto, $langFrom);
