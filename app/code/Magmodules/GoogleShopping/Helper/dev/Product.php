...
class Product extends AbstractHelper
{
    ...
    public function validateProduct($product, $parent, $config)
    {
        ...
        if (!empty($parent)) {
            if (!empty($filters['stock'])) {
              if (($parent->getSecondeqty() == 0) && (($parent->getQty() == 0))){
                  return false;
              }
    ...
    /**
     * @param \Magento\Catalog\Model\Product $pr
     *
     * @return int
     */
    public function getStockstatus($pr)
    {
      return 1;
      try{
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $productRepository = $objectManager->get('\Magento\Catalog\Model\ProductRepository');
        $prod = $productRepository->get($pr);

        //$prod = $objectManager->create('Magento\Catalog\Model\ProductRepository')->load($pr);
        if (($prod->getSecondeqty() > 0) || (($prod->getQty() > 0))){
            return 1;
        }else{
          return 0;
        }
      } catch (\Exception $e) {
          $this->logger->add('stock', $e->getMessage());
      }
    ...
    public function checkBackorder($product, $inv)
    {
        ...
        if (($product->getSecondeqty() == 0) && (($product->getQty() == 0))){
            return false;
        }
