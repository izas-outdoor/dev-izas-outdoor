 ...
 */
class OrderInfo extends \WebPanda\Rma\Controller\Guest
{
    /**
     * @var OrderFactory
     */
    ...
    {
        return $this->_url->getUrl('sales/guest/view', ['order_id' => $orderId]);
    }
        ...
        foreach ($openRmaItems as $item) {
            ...
            if (array_key_exists($item->getOrderItemId(), $returnsForItem)) {
                $returnsForItem[$item->getOrderItemId()][$item->getRmaId()] = [
                    'id' => $item->getRmaId(),
                    'url' => $this->_url->getUrl('rma/guest/view', ['id' => $item->getRmaId()]),
