...
class ProductAttribute(models.Model):
    ...
    def check_display_type(self):
        for rec in self:
            TypeDisplay = self.search([('display_type', '=', 'size'),('id', '!=',rec.id)])
            if TypeDisplay:
                raise ValidationError(_("You can't select multiple time 'type of size' !!!"))
...
class QuickOrder(models.Model):
    ...
    def compute_total(self):
        for rec in self:
            total = 0.0
            for r in rec.quick_order_line:
                T = r.quantity * r.product_id.lst_price
                total += T
            rec.total = total
    ...
    def create(self, vals):
        ...
        if not vals.get('name'):
            ...
            if len(postfix) == 1:
                ...
                name = 'QO'+postfix
            result.name = name
        quick_order = self.search([('state', '=', 'draft'),('user_id', '=', self._uid),('type','=',order_type)])
