...
class MagentoInstance(models.Model):
    ...
    def synchronize_metadata(self):
        ...
        """
        for record in self:
            print(' record.sync_price_scop() 1')
            record.sync_price_scop()
            print(' record.sync_price_scop() 2')
            
            record.import_currency()
            print(' record.sync_price_scop() 3')
            
            record.sync_website()
            print(' record.sync_price_scop() 4')
            
            record.sync_storeview()
            print(' record.sync_price_scop() 5')
            
            record.import_payment_method()
            print(' record.sync_price_scop() 6')
            
            record.import_delivery_method()
            print(' record.sync_price_scop() 7')
            
            record.import_magento_inventory_locations()
            print(' record.sync_price_scop() despues de este interesa')
    ...
    def open_report(self, record_id):
        ...
        action.update({'context': {'search_default_magento_instances': record_id, 'search_default_Sales': 1,
    ...
    def get_compared_year_data(self, record):
        ...
        return current_total, previous_total
    stock_min = fields.Integer(string="Stock minimo", help="Cantidad de stock minimo sube a magento si es igual o inferior a este número edita el producto a no tiene stock en magento")