    ...
    def create(self, vals):
        ...
        if self.user_has_groups('odoo_magento2_ept.group_magento_user_ept'):
            ...
            if vals.get("product_id", False):
                self.create_magento_variant_product_images(result, magento_product_image_vals, vals)
                ...
                for magento_template in magento_templates:
                    existing_gallery_images = magento_product_template_obj.browse(
                        magento_template["id"]).magento_product_image_ids.filtered(lambda x: not x.magento_product_id)
                    sequence = 1
